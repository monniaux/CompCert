...
Require Import TreeProperties Lia.
...
Lemma generate_moves_respects:
  forall moves : list  (reg * reg),
  forall jump_point : node,
  forall already : code,
  forall next_node : node,
  forall BOUND: strictly_bounds_tree next_node already,
    let (generated, next_node') := generate_moves moves jump_point already next_node in
    next_node' > next_node /\
    forall pc, pc < next_node -> generated!pc = already!pc.
Proof.
  induction moves; simpl.
  { intros.
    split. lia.
    intros.
    apply PTree.gso.
  ...
  intros.
  set (instr := (Iop Omove (src :: nil) dst (Pos.succ next_node))).
  assert (strictly_bounds_tree (Pos.succ next_node)
                               (PTree.set next_node instr already)) as BOUND'.
  {
    apply forall_tree_set.
    lia.
    apply forall_tree_implies with (P := fun key val => key < next_node).
    intros; lia.
    exact BOUND.
  }
  pose proof (IHmoves jump_point  (PTree.set next_node instr already)
                      (Pos.succ next_node) BOUND') as REC.
  destruct (generate_moves moves jump_point (PTree.set next_node instr already)
                           (Pos.succ next_node)) as (generated, next_node').
  destruct REC as (NEXT, PRESERVES).
  split.
  lia.
  intros.
  rewrite PRESERVES by lia.
  apply PTree.gso.
  lia.
Qed.
