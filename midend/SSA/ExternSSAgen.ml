...
let rename_instr node rename_def rename_use = function
  ...
  | Ibuiltin(ef, args, res, s) ->
      SSA.Ibuiltin(ef, List.map rename_use args, rename_def res, s) 
...
let succ f = 
  let map = RTL.successors_map f in
    fun i -> 
...
let succ_and_pred f = 
  ...
  let pred = Kildall.make_predecessors (fn_code f) RTL.successors_instr in
    ((fun i -> 
...
let entry f =
  int_of_positive f.RTL.fn_entrypoint

let size f = 
  1+ int_of_positive (RTLnorm.get_max f.RTL.fn_code)
...
let var_def  = function
  ...
  | Some ins ->
	...
	| Iload (_,_,_,r,_) 
	| Icall (_,_,_,r,_)
	| Ibuiltin (_,_,r,_) -> Ptset.singleton (int_of_positive r)
	| _ -> Ptset.empty
...
let all_vars f = 
  PTree.fold
	...
	| Ibuiltin (_,l,r,_) -> add_list (r::l) set
...
let remove_dead_node f =
  ...
  let rec dfs v =
    ...
    dfs f.RTL.fn_entrypoint;
    let new_code = 
      PTree.fold
	(fun new_code n ins ->
	   if has_been_seen n then new_code else PTree.remove n new_code)
	f.RTL.fn_code f.RTL.fn_code in
      { RTL.fn_sig = f.RTL.fn_sig;
	RTL.fn_params = f.RTL.fn_params; 
        RTL.fn_stacksize = f.RTL.fn_stacksize;
	RTL.fn_code = new_code;
	RTL.fn_entrypoint = f.RTL.fn_entrypoint;
	RTL.fn_dfs = []  
      }
...
let genSSA_V2 f live = 
  ...
  let params = List.map int_of_positive f.fn_params in
  let (rename_def,rename_def_phi,max_index) = rename_V2 size entry params f.fn_code 
