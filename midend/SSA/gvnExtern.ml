...
let extern_gvn (f':SSAtool.coq_function) (register_dom_test:ssa_def->node->unit) : ssa_def Utils.P2Tree.t =
  let f = f'.SSAtool.func in
  let _ = if debugmode then print_string "starting extern_gvn\n" in
  SSAvarHash.init ();
  let eqs = build_equations f in
    if Ptmap.is_empty eqs then Utils.P2Tree.empty
    else
      let g = Graph.make f f'.SSAtool.tool.SSA.fn_dom_test eqs in
			   ...
			   (fun repr pc ins ->
				...
				| Iop (op, args, res, s) when not (Op.op_depends_on_memory op) -> 
				  ...
				  | Some (x',d') -> 
					...
					Utils.P2Tree.set res d' repr;
				      end
				    else repr
				  | _ -> repr)
				| _ -> repr) (fn_code f) Utils.P2Tree.empty in
      (* Printf.printf "%d\n" !count_optim; *)
      representant

let extern_gvn2 (f':SSAtool.coq_function) : ssa_def Utils.P2Tree.t =
