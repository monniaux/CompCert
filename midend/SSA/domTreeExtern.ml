...
module DomTree = struct
  ...
  let successors_map f =
    PTree.map1 SSA.successors_instr f.fn_code

  open ExternSSAgen
 
  let succ_and_pred f = 
    let succ = successors_map f in
    let pred = Kildall.make_predecessors (fn_code f) SSA.successors_instr in
    ((fun i -> 
  ...
  let size f = 
    1+ int_of_positive (get_max f.SSA.fn_code)
