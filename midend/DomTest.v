...
(* Will be replace during extraction by [let rec fuel = S fuel] *)
Axiom fuel : nat.
...
(* Calcul externe donnant D sous forme de liste d'association (fils,père)
   ce calcul doit assurer pour la completude du checker, que la liste suit un tri topo *)
Axiom extern_d : function -> list (node * node).

(* Validateur des contraintes dataflow grâce au théorème de correction *)
Definition check_D_eq (f : function) (D: node -> node) (test_dom: node -> node -> bool) : bool :=
  if (peq (D (fn_entrypoint f)) (fn_entrypoint f)) then
    forall_ptree (fun pc spc => forallb (test_dom pc) spc) (successors f)
  else false.

Lemma check_D_eq_correct : forall f sn itvm,
  let l := extern_d f in 
  let D := make_D_fun l in 
   build_succs (entry f) l = Some sn -> 
   build_itv_map (entry f) sn = Some itvm ->
   check_D_eq f D (fun i j => is_ancestor itvm (D j) i) = true -> 
  D_spec (fn_entrypoint f) (SSA.cfg f) D.
Proof.
  unfold check_D_eq.
  intros.
  flatten H; intuition. 
  ...
  intros. invh cfg. 
  set (fpt := (fun pc succ_pc => forallb (fun j => is_ancestor itvm0 ((make_D_fun (extern_d f) j)) pc) succ_pc)) in *.
...
(* Calcul du test de dominance *)  
Definition compute_test_dom (f: function) : option (node -> node -> bool) :=
  let l := extern_d f in 
  let D := make_D_fun l in 
  match build_succs (entry f) l with  
      | Some sn => (* sn : D renversé sous forme de map *)
        match build_itv_map (entry f) sn with 
            | Some itvm => (* itvm : pour chaque noeud, son intervalle *)
              let td := (fun i j => is_ancestor itvm (D j) i) in 
              if (check_D_eq f D td) then Some (is_ancestor itvm)
              else None
            | None => None
        end
      | None => None
...
Proof.
  ...
  eapply D_spec_correct with (D:= (make_D_fun (extern_d f))); eauto.
