...
Definition compute_dom (f: function): option (PMap.t L.t) :=
  DS.fixpoint f.(fn_code) successors_instr (transfer f) 
                          ((f.(fn_entrypoint), singleton f.(fn_entrypoint)) :: nil).
...
Proof.
  ...
  assert (L.ge mdom!!pc' (transfer f pc mdom!!pc)) by (eapply DS.fixpoint_solution; eauto).
...
(* Not used any more because it is superseed by DomTest

Inductive test_dom_type := TD (test:node -> node -> bool) | TDerror.

Definition dom_test' (f:function) :=
  match compute_dom f with
    | Some mdom => TD (fun d n => in_set d mdom!!n)
    | None => TD (fun d n => false)
  end.

Definition dom_test (f:function) :=
  match dom_test' f with
    | TD f => f
    | TDerror => (fun d n => false)
  end.
